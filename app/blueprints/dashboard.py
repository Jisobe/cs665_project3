...
def dashboard_summary():
    ...
    vitals_avg=db.session.query(
        func.round(func.avg(Vitals.height), 0).label("avg_height"),
        func.round(func.avg(Vitals.weight), 0).label("avg_weight"),
        ...
        func.round(func.avg(Vitals.temperature), 0).label("avg_temp"),
        func.round(func.avg(Vitals.pain_level), 0).label("avg_pain"),
    )
    ...
    diagnosis_by_status=db.session.query(
        Diagnosis.status,
    ...
    return jsonify({
        ...
        "visit_per_provider":[
            {
                ...
                "name": f"{row.first_name} {row.lst_name}",
        ...
        "diagnosis_by_status":[
            {"status": row.status,
             "count": row.count
            } for row in diagnosis_by_status
